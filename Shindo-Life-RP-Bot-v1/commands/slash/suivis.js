    ...
    async execute(interaction, client, config) {
        ...
        const generateEmbed = (page) => {
            ...
            const fields = currentEntries.map((entry, index) => {
                ...
                // CORRECTION ICI : Convertir validation_date en objet Date avant d'appeler getTime()
                value += `*Validé le:* <t:${Math.floor(new Date(entry.validation_date).getTime() / 1000)}:D>\n`;
